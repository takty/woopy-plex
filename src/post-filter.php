...
class PostFilter {
	...
	private $_var_to_taxonomy = array();
	private $_post_types = array();
	private $_filtered_taxonomies = array();
	...
	public function add_filtered_post_type( $post_type_s ) {
		...
		foreach ( $post_type_s as $pt ) {
			foreach ( $this->_var_to_taxonomy as $tax ) {
				register_taxonomy_for_object_type( $tax, $pt );
			}
		}
	...
	public function initialize() {
		if ( is_admin() ) {
			add_action( 'edited_term_taxonomy', array( $this, '_cb_edited_term_taxonomy' ), 10, 2 );
		} else {
			add_filter( 'get_next_post_join',      array( $this, '_cb_get_adjacent_post_join' ), 10, 5 );
			add_filter( 'get_previous_post_join',  array( $this, '_cb_get_adjacent_post_join' ), 10, 5 );
			add_filter( 'get_next_post_where',     array( $this, '_cb_get_adjacent_post_where' ), 10, 5 );
			add_filter( 'get_previous_post_where', array( $this, '_cb_get_adjacent_post_where' ), 10, 5 );

			add_filter( 'getarchives_join',        array( $this, '_cb_getarchives_join' ),  10, 2 );
			add_filter( 'getarchives_where',       array( $this, '_cb_getarchives_where' ), 10, 2 );

			add_action( 'posts_join',              array( $this, '_cb_posts_join' ),  10, 2 );
			add_action( 'posts_where',             array( $this, '_cb_posts_where' ), 10, 2 );
			add_action( 'posts_groupby',           array( $this, '_cb_posts_groupby' ), 10, 2 );

			\st\custom_rewrite\add_post_link_filter( array( $this, '_cb_filter_by_taxonomy' ) );
		}
	}
	...
	private static function _make_join_term_relationships( int $count, string $type = 'INNER' ): string {
		global $wpdb;
		$q = array();
		for ( $i = 0; $i < $count; $i += 1 ) {
	...
	private static function _make_where_term_relationships( array $term_taxonomies ): string {
		global $wpdb;
		$q = array();
	...
	private function _get_term_taxonomy_ids(): array {
		static $ret = null;
		if ( $ret ) {
			return $ret;
		}
		...
		foreach ( $vars as $var ) {
			if ( ! isset( $this->_var_to_taxonomy[ $var ] ) ) {
				continue;
			}
	...
	public function _cb_get_adjacent_post_join( string $join, bool $in_same_term, $excluded_terms, string $tax, \WP_Post $post ): string {
	...
	public function _cb_get_adjacent_post_where( string $where, bool $in_same_term, $excluded_terms, string $tax, \WP_Post $post ): string {
		if ( ! in_array( $post->post_type, $this->_post_types, true ) ) {
			return $where;
		}
		$tts = $this->_get_term_taxonomy_ids();
	...
	public function _cb_getarchives_join( string $join, array $r ): string {
		if ( ! is_main_query() ) {
			return $join;
		}
		if ( ! in_array( $r['post_type'], $this->_post_types, true ) ) {
			return $join;
		}
		$tts = $this->_get_term_taxonomy_ids();
		if ( ! empty( $tts ) ) {
	...
	public function _cb_getarchives_where( string $where, array $r ): string {
		if ( ! is_main_query() ) {
			return $where;
		}
		if ( ! in_array( $r['post_type'], $this->_post_types, true ) ) {
	...
	public function _cb_posts_join( string $join, \WP_Query $query ): string {
		if ( ! is_main_query() || empty( $this->_post_types ) ) {
			return $join;
		}
		$tts = $this->_get_term_taxonomy_ids();
		if ( empty( $tts ) ) {
	...
	public function _cb_posts_where( string $where, \WP_Query $query ): string {
		if ( ! is_main_query() || empty( $this->_post_types ) ) {
			return $where;
		}
		$tts = $this->_get_term_taxonomy_ids();
	...
	public function _cb_posts_groupby( string $groupby, \WP_Query $query ): string {
		if ( ! is_main_query() ) {
			return $groupby;
		}
		if ( is_search() ) {
	...
	public function _cb_filter_by_taxonomy( array $vars, \WP_Post $post = null ): array {
	...
	public function _cb_edited_term_taxonomy( int $term, string $taxonomy ) {
		if ( empty( $this->_filtered_taxonomies ) ) {
			return;
		}
		$taxes = array();
		$slugs_array = array();
		foreach ( \st\custom_rewrite\get_structures() as $st ) {
			if ( ! isset( $this->_var_to_taxonomy[ $st['var'] ] ) ) {
				continue;
			}
		...
		if ( $is_filtered ) {
			...
			$tars = get_terms( $this->_filtered_taxonomies, array( 'hide_empty' => false ) );
		}
		global $wpdb;
		$pts = "('" . implode( "', '", $this->_post_types ) . "')";
